...
const STATE_FILE = path.join(config.dataDir, "processed-cards.json");
const ATTEMPTS_FILE = path.join(config.dataDir, "failed-attempts.json");
...
export async function loadProcessedCards(): Promise<void> {
  ...
  try {
    ...
    log.info(`Loaded ${processedCards.size} processed card IDs`);
  } catch (err: any) {
    if (err.code === "ENOENT") {
